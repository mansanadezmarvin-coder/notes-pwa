...
function collectData(){
  const get = id => { const el = $id(id); return el ? el.value : '' }
  ...
  return {
    ...
    age: (get('age') || '').trim(),
    linkedin: (get('linkedin') || '').trim(),
    address: (get('address') || '').trim(),
    citycountry: (get('citycountry') || '').trim(),
    summary: (get('summary') || '').trim(),
    skills: splitCSV(get('skills')),
    experience: splitLines(get('experience')),
    education: splitLines(get('education')),
    certifications: splitLines(get('certifications')),
    projects: splitLines(get('projects')),
    languages: splitLines(get('languages')),
    awards: splitLines(get('awards'))
  }
}
